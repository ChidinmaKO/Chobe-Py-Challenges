...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    #     state_abbrev = [NOT_FOUND if state_name not in us_state_abbrev.keys() else us_state_abbrev[state_name]]
...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
    max_len = max([len(state) for state in states or state in us_state_abbrev.keys() if data == states or data == us_state_abbrev])
    for state in data:
    ...
    # else:
    #     max_len = max([len(state) for state in us_state_abbrev.keys()])
    # for state in data:
    #     if len(state) == max_len:
    #         return state
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,states=states):
    """Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
       ...
       alphabetical order"""
    first_10_abbrev = sorted([v for v in us_state_abbrev.values()])[:10]
    last_10_states = sorted([state for state in states])[-10:]
