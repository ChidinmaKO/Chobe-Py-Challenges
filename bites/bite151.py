...
def operas_both_at_premiere(guest, composer):
    ...
    """
    if guest not in composers or composer not in composers or guest == composer:
        raise ValueError

    composed_operas = [opera for opera in operas if opera.author == composer]
    ...
    for composed_opera in composed_operas:
        composed_opera_date = _get_date(composed_opera.date)
        if guest_born <= composed_opera_date <= guest_died\
                and composer_born <= composed_opera_date <= composer_died:
            yield composed_opera.play
