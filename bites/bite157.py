...
def filter_accents(text):
    ...
    """
    # result = set(char.lower() for char in text if not char.isascii())
    # result = { char.lower() for char in text if unicodedata.normalize('NFD', char) != char }
    result = { char.lower() for char in text if unicodedata.decomposition(char) }
    return result
